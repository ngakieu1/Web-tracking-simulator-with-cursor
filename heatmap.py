...
def Fixpos2Densemap(fix_arr, width, height, imgfile=None, alpha=0.5, threshold=10):
    ...
    if imgfile.any():
        ...
        return heatmap
def generate_heatmaps(username, img_path='static/newspp.jpg', csv_file=None, output_folder='static/heatmaps'):
    if csv_file is None:
        csv_file = f'static/mouse_data/{username}/mouse_data.csv'

    if not os.path.exists(csv_file):
        print(f"CSV file not found for user: {csv_file}")
        return None
    ...
    fix_arr = df[['gaze_x', 'gaze_y', 'fixation_duration']].values
    fix_arr[:, 0] = np.clip(fix_arr[:, 0], 0, W)
    fix_arr[:, 1] = np.clip(fix_arr[:, 1], 0, H)

    heatmap = Fixpos2Densemap(fix_arr, W, H, imgfile=img_file, alpha=0.7, threshold=5)

    os.makedirs(output_folder, exist_ok=True)
    output_filename = f"{output_folder}/{username}.png"
